...
const ImageToGrayscale: React.FC = () => {
  ...
  const sampleImages = [
    { src: "/colorfull-images/colorfull (2).webp", alt: "Coin" },
    { src: "/colorfull-images/colorfull (3).webp", alt: "Portrait 1" },
    { src: "/colorfull-images/colorfull (4).webp", alt: "Phone" }
  ];
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      setImageUrl(null); // Clear the result when a new file is uploaded
    }
  };
  ...
  const handleSampleClick = (src: string) => {
    ...
    setImageUrl(null); // Clear the result when a new sample image is selected
  };
  ...
  const handleStartAll = async () => {
    if (selectedFile || selectedSampleImage) {
      ...
      try {
        const response = await axios.post('http://localhost:8000/grayscale', formData, {
          headers: {
    ...
    <div>
      ...
      <div className="mx-auto max-w-screen-md">
        ...
        <p className="mb-8 text-base font-light flex justify-center text-gray-500">
          Convert your JPG, PNG images into grayscale form.
        </p>
...
export default ImageToGrayscale;
